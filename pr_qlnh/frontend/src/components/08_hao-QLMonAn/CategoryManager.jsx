...
export default function CategoryManager() {
    ...
    const handleDelete = async (id, name) => {
        ...
        try {
            ...
            console.error("Lỗi khi xóa danh mục:", err.response ? err.response.data : err.message);
        ...
        <div className="dish-layout">
            ...
            <main className="dish-main">
                <div className="dish-container p-6">
                    <h2 className="text-2xl font-semibold text-gray-800 mb-6">Quản Lý Danh Mục Món Ăn</h2>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-64">Tên Danh Mục</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Slug (Đường dẫn)</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Số món ăn</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Trạng thái</th>
                                        <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-32">Thao Tác</th>
                                    </tr>
                                </thead>
                                    ...
                                    {categories.length > 0 ? (
                                        categories.map((cat, index) => (
                                            <tr key={cat.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{cat.dishesCount}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                                        ${cat.isHidden ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}
                                                    >
                                                        {cat.isHidden ? 'Ẩn' : 'Hiển thị'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center space-x-2">
                                                    <button onClick={() => handleEdit(cat)} className="text-indigo-600 hover:text-indigo-900" title="Chỉnh sửa">
                                                        {/* Icon Edit */}
                                        ...
                                        <tr>
                                            <td colSpan={6} className="category-no-data text-center py-4 text-gray-500">
